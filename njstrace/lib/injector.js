...
var ON_CATCH = 'if (__njsOnCatchClause__) {\nif (typeof __njsEntryData__ !== "undefined") __njsOnCatchClause__({entryData: __njsEntryData__});\n}';
...
Injector.prototype.injectTracing = function (filename, code, wrapFunctions, includeArguments, wrappedFile) {
	...
	var output = falafel(code, { ranges: true, locations: true, ecmaVersion: 9 }, function processASTNode(node) {
		...
		if (name && node.body.type === Syntax.BlockStatement) { // Not supporting arrow functions with no body
			...
			///hacking - put some code at the begin and of the function
			
			if(global.NR_CONTROLLER_INSTRUMENTATION) {
				newFuncBody = origFuncBody;
				wrapFunctions = false;	
				if(global.NR_RECURSION_BODY) {
					newFuncBody = global.NR_RECURSION_BODY;
					global.NR_RECURSION_BODY = undefined;
				}
			}

			if (global.NR_HACK_NJS_SHOULD_INSTRUMENT
				&& global.NR_HACK_NJS_SHOULD_INSTRUMENT(filename, name, startLine, node)) {				
				if (global.NR_HACK_NJS_ENTRY) {
					newFuncBody =  global.NR_HACK_NJS_ENTRY + newFuncBody;
				}
					

				if (global.NR_HACK_NJS_EXIT) {
					newFuncBody = newFuncBody + global.NR_HACK_NJS_EXIT;
				}

				// console.log('--------------------------------------');
				// console.log(funcDec + '{' + newFuncBody + '}');
				// console.log('--------------------------------------');	
			}
			///hacking
